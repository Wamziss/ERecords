...
function Sidebar() {
    ...
    useEffect(() => {
        ...
        const index = ['Files', 'Shared', 'Archived', 'Settings', 'Home'].indexOf(path.split('/')[1]);
        if (index !== -1) {
                ...
                <ul className="nav flex-column p-4">
                    {['Files', 'Shared', 'Archived'].map((item, index) => (
                        ...
                        >
                            ...
                            >
                                <i className={`bi ${index === 0 ? 'bi-folder-fill' : index === 1 ? 'bi-folder-symlink-fill' : 'bi-archive-fill'}`} style={styles.icon}></i>{item}
                            </a>
                        </li>
                    ))}
                </ul>
